...
class PttWebCrawler(object):
            ...
            for i in range(end-start+1):
                ...
                for div in divs:
                    try:
                        ...
                        jsonObj = self.parse(link, article_id, board)
                        
                        jsonstring += json.dumps(jsonObj, sort_keys=True, ensure_ascii=False) + '\n'
                        filenamePrefix = board + '-' + article_id
                        
                        filename = filenamePrefix + '-' + str(jsonObj['message_count']['all']) + '.json'
                        filename = os.path.join(path, filename)
                        # 檢查是否未異動，依據推文數來看
                        if os.path.isfile(filename):
                            # print('post not modified: ' + filename)
                            continue
                        
                        # 如果有異動，則檢查是否已經抓過，把舊的移除
                        findOldFile = glob.glob(os.path.join(path, filenamePrefix + '*'))
                        if len(findOldFile) > 0:
                            # dpath = os.path.join(path, findOldFile[0])
                            print('delete file: ', findOldFile[0])
                            os.remove(findOldFile[0])

                        self.store(filename, jsonstring, 'w')
                        time.sleep(1)
    ...
    def parse_article(self, article_id, board, path='.'):
        ...
        filename = os.path.join(path, filename)
        jsonObj = self.parse(link, article_id, board)
        self.store(filename, json.dumps(jsonObj, sort_keys=True, ensure_ascii=False), 'w')
        return filename
    ...
    def parse(link, article_id, board, timeout=3):
        ...
        for push in pushes:
            ...
            messages += push_tag + u'_' + push_userid + u':' + push_content + '_' + push_ipdatetime + ';\n'
            if push_tag == u'推':
                p += 1
            elif push_tag == u'噓':
                b += 1
